...
def calculate_zipfs_law(words):
    ...
    # Sort by frequency using iterative approach
    # sorted_word_counts = []
    # for word, frequency in word_counts.items():
    #     inserted = False
    #     for i in range(len(sorted_word_counts)):
    #         if frequency > sorted_word_counts[i][1]:
    #             sorted_word_counts.insert(i, (word, frequency))
    #             inserted = True
    #             break
    #     if not inserted:
    #         sorted_word_counts.append((word, frequency))
